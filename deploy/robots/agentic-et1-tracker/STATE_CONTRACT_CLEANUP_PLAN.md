...
| F4 | passive 默认密码示例 | README 暴露 `{"password":"galaxy"}` 默认示例，与 skill 的“operator 必须显式提供密码、agent 不查默认密码”冲突。 | 修 README 表述为 `{"password":"<operator-provided-password>"}`，说明 password 由 operator 显式提供；不做密码系统重构。 | 文档 |
...
| F7 | `et1-action` standby 误判 | `standby`/`idle-clear` 可能把 `ctrl:"fixstand"` 且无 active 的瞬时状态误判为 standby 成功。 | 独立最小代码修复：只改一个确认函数 `standby_confirmed()`，并让 `standby` 与 `idle-clear` 复用同一 standby 判断；`idle-clear` 另加 idle config 已清空证明。 | 代码 |
...
- 更新 README `/idle` 段落：
  ...
  - 区分 HTTP `/idle {"paths":[]}` 任意状态清空与 CLI `idle-clear` 的 clear-then-standby 行为；CLI 成功仍需要 ordinary standby/idle 确认。
...
- 更新 README 状态矩阵：
  ...
  - passive 示例改为 operator-provided password，不出现默认密码字面量。
...
- 更新 skill `references/output-contract.md`：
  ...
  - 补充 `idle-clear` 先发送 HTTP clear，再请求 ordinary `/standby`；不能 standby 的状态可能让 CLI 失败，但 clear request 已发出。
...
该交付可独立于纯文档 cleanup 合入。只修改 `et1-action` 中一个 standby 确认函数 `standby_confirmed()`，不要为 `standby` 和 `idle-clear` 分两套 standby 判断；`idle-clear` 在复用 standby 判断后，再验证 idle config 已清空。
...
- `idle-clear` 复用同一个 `standby_confirmed()`，但成功还必须证明 idle config 已清空；不能只靠 active idle 确认，也不能返回 fixstand 瞬态作为成功。
...
- `et1-action idle-clear` 与 `standby` 复用同一 standby confirmation 逻辑，并且必须证明 idle config 已清空后进入 standby；不返回 fixstand 成功，也不把 active idle 当作清空成功。
- ACCEPTANCE 显式覆盖：
  - non-empty `/idle` accepted/rejected 状态。
  - non-empty `/idle` blocked controller 先于 readiness/manual error。
  - HTTP `/idle {"paths":[]}` 任意状态清空。
  - CLI `idle-clear` 先 clear 再 ordinary standby；不能 standby 的状态可失败，不能写成任意状态成功。
...
| `standby_confirmed()` 收紧后 standby 更容易返回未确认 | CLI 可能在瞬态状态下多等或返回 `STANDBY_NOT_CONFIRMED` | 拒绝 unsafe/terminal、user/transition、字段不足状态；只接受 active idle 或纯 standby；idle-clear 另证 idle config cleared | 回滚单函数改动 |
...
- 不为 `standby` 和 `idle-clear` 写两套确认逻辑。
