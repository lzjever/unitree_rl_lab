...
本文档交付定位是行为基线 + 下一步 P1 实现计划，不是全量长期 roadmap。

| 标记 | 含义 | Handoff 用法 |
| --- | --- | --- |
| Current | 当前代码已经实现或 HTTP/API 已经暴露的合同；文档必须与代码一致。 | 必须保持，作为回归锁定。 |
| Target/P1 | 下一步唯一主目标；当前可能尚未实现。 | 允许改 runtime/test 来达成，但不新增 API、不改 ET1、不扩大 `.trk` 格式。 |
| Backlog/P2 | 有价值但不进入下一步交付的改进或补测。 | 不阻塞 P1，不应驱动范围蔓延。 |

优先级含义：

| Priority | 含义 |
| --- | --- |
| P0 | 当前安全/API 合同或已实现行为回归锁定；不能被 P1 改动破坏。 |
| P1 | 下一步目标，只聚焦 background transition/playback 下 user `.trk` queue 抢占背景，以及 target-specific interrupt/status/API 回归。 |
| P2 | Backlog；可测试性、边界一致性或 operator 体验改进，不进入下一步主实现。 |
...
| 目标 | 结论 |
| --- | --- |
| background transition/playback 下 user queue 必须抢占背景 | `transition.target=idle`、`transition.target=standby`、standby playback 收到 queue user `.trk` 时，从 current reference 平滑切到 user，不等背景完成。 |
...
| background queue 抢占失败语义最小化 | target user load/align/transition build 失败时 publish target `Failed`、`stop_reason=None`；old idle/standby target 不写 user history，不恢复旧 background transition。 |
...
| Public `ControllerState::Idle` | 对外 `ctrl:"idle"` | 当前 `/execute` 也会被 controller gate 拒绝；它不是 user `.trk` 的 no-active accepting state，no-active user path 应走 public `standby_velocity`。 |
...
| user interrupt | `POST /execute {"mode":"interrupt"}` | 紧急用户 `.trk`，当前只应抢占 waiting/background；active user running 走 controlled stop。 |
...
## 3. Current Behavior + Target Recommendation Matrix

本节的 Delivery class 以开头 Reading contract 为准。P1 只表示下一步主目标；已实现的 `/standby_velocity`、`/fixstand`、idle config 行为作为 Current/P0 或 P2 regression lock 保留，不作为下一步主实现方向。
...
| public `ctrl:"idle"` | user queue / interrupt | `/execute` 先过 readiness gate；若 readiness OK，controller gate 返回 `CONTROL_STATE_CONFLICT` / `wrong ctrl; check /status`；不调用 validator/sink/id generator。 | 是 | 保持。public `idle` 不是 user motion accepting state；用户应等 status 回到可接受的 `standby_velocity`/running path。 | P0 | 补/保留 public `ControllerState::Idle` execute gate 测试。 |
...
| active idle | user queue | 当前直接尝试 `idle current frame -> user first frame` synthetic transition；成功后 `active.kind:"transition"`、`transition.target:"user"`、`target_id` 为 user run id；失败时发布 user failed，停止 idle 并清空 idle config。 | 是 | 保持。queue 对 active idle 已经是已实现抢占。 | Current | 已有 active idle preempt 和 failure 测试。 |
...
| active user preparing | user interrupt | 取消 waiting；urgent 入 waiting；当前 active 被标记 `Stopping`，进入 `Stopping`，之后 urgent 从 standby/no-active 规则启动。不是 current-frame transition。 | 是，保守 | 保持 controlled stop。不要默认宣称 active user 可直接 current-frame transit；未来除非有 safety gate。 | P0 | 建议补 preparing interrupt controlled-stop 专门测试。 |
...
| `transition -> user` | user queue | 新 user 只进入 waiting；不中断当前 transition，旧 target user 仍优先。 | 是 | 保持。当前 transition 已经归属某个前景 user target，新 queue 不应抢占它。 | Current | Required for next change 只需锁定 target-specific 行为。 |
| `transition -> user` | user interrupt | 取消 waiting；取消旧 target user；从当前 transition frame 重新构建到 urgent user 的 transition。 | 是 | 保持。interrupt 应覆盖旧 user target。 | Current | Required for next change 中作为 target-specific interrupt 回归。 |
| `transition -> idle` | user queue | 当前只入 waiting，不打断 transition；transition 完成后仍启动 target idle，不先看 waiting；queued user 可能还要等 idle playback 完成。 | 否 | queue 也应抢占背景 idle target：取消 idle target，从当前 transition frame transition 到 user。 | P1 | 缺口：需要新增 `transition.target=idle` + queue 抢占测试。 |
| `transition -> idle` | user interrupt | 通用 active transition interrupt 会从当前 transition frame 重新构建到 urgent user。 | 目标合理；测试不足 | 保持，并补专门 target=idle 测试。 | P1 | 缺口：transition->idle interrupt 专门测试。 |
| `transition -> standby` synthetic | user queue | 当前只入 waiting，不打断 transition；transition 完成后仍启动 standby playback，不先看 waiting；queued user 可能等 playback 完成。 | 否 | queue 应抢占背景 standby target：从当前 transition frame transition 到 user。 | P1 | 缺口：新增 queue 抢占 `transition.target=standby` 测试。 |
| `transition -> standby` synthetic | user interrupt | 通用 active transition interrupt 会从当前 frame 到 urgent user；但缺 target=standby 专门覆盖。 | 目标合理；测试不足 | 保持，并确认旧 standby target 被丢弃且不播放 standby asset。 | P1 | 缺口：transition->standby interrupt 专门测试。 |
| standby playback as `ActiveKind::Transition` | user queue | 当前只入 waiting；已在 waiting 的 user 不会自动抢占，standby playback 继续播放完。 | 否 | queue 应抢占背景 standby playback：从当前 playback frame transition 到 user。 | P1 | 缺口：standby playback queue 抢占测试。 |
| standby playback as `ActiveKind::Transition` | user interrupt | 通用 transition interrupt 应可从当前 playback frame 到 urgent user。 | 目标合理；测试不足 | 保持，并补 standby playback interrupt 测试。 | P1 | 缺口：standby playback interrupt current-frame 测试。 |
...
| active idle | `/standby_velocity` | `stopIdleActive()`；idle config 保留，回到可播放 idle 的 standby 链路后，若 idle config 非空且无 user work、ready/safe，idle 可按现有自动播放规则重新启动。 | 是 | 保持。`/standby_velocity` 不清 idle config、不生成 user run、不进 queue；后续 idle 启动是 background idle manager 的正常行为，不是 `/standby_velocity` 队列化。若 caller 需要纯 `standby_velocity` 且不播放 idle，应先 `/idle {"paths":[]}` 或用 `/stop`。 | P2 | 保留/补测试锁定现有行为：active idle 停止、idle config 保留、满足自动播放条件时可重新 idle。 |
...
| transition target idle | transition completion | target idle 进入 active idle；即使已经有 waiting user，也不先看 waiting；无 user id/history。 | 部分合理 | 背景 target 不写 history 这点保持；queue 应在 completion 前抢占，避免 waiting user 等目标 idle 播放完成。 | P1 | 已有 user->idle/idle->idle；补 queue 抢占背景目标后完成测试。 |
| transition target standby | transition completion | 如果 target has standby track，则进入 standby playback，仍用 `ActiveKind::Transition`；即使已经有 waiting user，也不先看 waiting；playback 完成后回 `GeneralTrackerIdle` / public standby。 | 部分合理 | 背景 target/playback 不写 history 这点保持；queue 应在 completion/playback 前抢占。 | P1 | 缺 queue/interrupt 抢占 standby playback 测试。 |
...
| Class | Gap | Why it matters | Required action |
| --- | --- | --- | --- |
...
| P1 | `transition -> idle` 中 user queue 当前不打断背景目标 | idle 是背景；当前 queue 只进 waiting，completion 后仍会启动 target idle，queued user 可能等 idle 播放完。 | 在 `CommandKind::Queue` + `ActiveKind::Transition` 时判断 target 是否 background；若 target idle，则从当前 transition frame 到 user。 |
| P1 | `transition -> standby` synthetic 中 user queue 当前不打断背景目标 | standby 是背景；当前 queue 只进 waiting，completion 后仍会启动 standby playback，queued user 可能等 playback 完。 | 同上，target standby 用 current transition frame 重建到 user。 |
| P1 | standby playback 中 user queue 当前不打断背景播放 | standby playback 仍是 background reference；已在 waiting 的 user 不会自动抢占 playback。 | 将 standby playback 视为 background transition target，queue/interrupt 都抢占。 |
| P1 | queue 抢占 background transition/playback 的失败语义必须锁住 | target user load/align/transition build 失败时不能恢复旧背景，也不能写 idle/standby history。 | target user publish `Failed`、`stop_reason=None`；abort 当前 background transition 或按现有 helper 合同进入 safe idle/standby；old background target 不写 user history。 |
| P1 | `transition -> idle/standby` interrupt 缺专门测试 | 通用 transition interrupt 已有，但没有锁住 background target 语义。 | 增加 target-specific runtime tests，断言 source frame 是被打断 transition 当前帧。 |
| P0 regression lock | active user running/preparing interrupt 不能被误改成 direct current-frame transition | 前景 user 可能有动态接触、policy hidden/history 风险；这是当前保守合同，不是 P1 新能力。 | 保留 controlled stop 测试，并在测试名中明确这是 intentional conservative behavior。 |
...
| `/standby_velocity` | control path；不清 idle config；不生成 user run、不进 queue；active user running/preparing controlled stop；静态 user holding 可走 held reference -> standby reference gated transition；active idle/transition abort。回到可播放 idle 的 standby 链路后，若 idle config 非空且无 user work、ready/safe，idle 可按现有 background idle manager 自动播放规则重新启动。需要纯 `standby_velocity` 时，caller 先 `/idle {"paths":[]}` 或使用 `/stop`。 |
...
| public `ctrl:"idle"` | `/execute` 也被 controller gate 拒绝；它不是 user `.trk` accepting state。 |
...
| idle failure | idle->user target load/validation fail | target user `Failed`；idle 停止；不恢复坏 idle；无 idle history。 |
...
| transition idle queue failure | `transition.target=idle` + queue user load/align/build fail | target user `Failed` with `stop_reason=None`；old idle target 无 user history；不恢复旧 background transition。 |
...
| transition standby queue failure | `transition.target=standby` + queue user load/align/build fail | target user `Failed` with `stop_reason=None`；old standby target 无 user history；不继续 standby playback。 |
...
| standby playback queue failure | standby playback active + queue user load/align/build fail | target user `Failed` with `stop_reason=None`；old standby playback 无 user history；按 helper 合同回 safe idle/standby。 |
...
- background queue 抢占失败：target user `Failed` with `stop_reason=None`；old idle/standby/standby playback 不写 user history；不恢复旧 background transition/playback。
...
Regression lock, not next main implementation:
...
## 8. Handoff Implementation Guidance
...
| Queue handling during transition | `CommandKind::Queue` 遇到 active transition 时，如果 target 是 background，应走 current-frame-to-user helper；如果 target 是 user，保持 waiting。 |
| Background preempt failure | queue 抢占 background transition/playback 时，若 target user load/align/transition build 失败，publish target `Failed`、`stop_reason=None`；abort 当前 background transition 或按现有 helper 合同进入 safe idle/standby；不要恢复旧 background transition；old idle/standby target 无 user history。 |
| Interrupt handling during transition | 保持当前 current-frame-to-urgent helper；补 target-specific tests。 |
...
| StandbyVelocity active idle | 保留 idle config；不新增 idle 保留但禁播的 runtime 状态。`/standby_velocity` 回到可播放 idle 的 standby 链路后，若 idle config 非空且无 user work、ready/safe，idle 可按现有 background idle manager 规则自动重新启动；这不是 `/standby_velocity` 队列化或生成 user run。需要纯 `standby_velocity` 时，caller 先 `/idle {"paths":[]}` 或使用 `/stop`。 |
| Tests | 先补 target-specific runtime tests，再改 runtime；API gate tests 确保 starting/public idle/passive/fixstand/stopping/fault 在 readiness OK 时拒绝 user `.trk`，且 `/execute` readiness 非 OK 时返回 readiness/manual error。另锁定 `/passive`：正确 password + readiness OK admitted、正确 password + bad_orientation exception admitted、无正确 password/lowcmd/manual/其他 readiness/fault error rejected，rejected path 不进 runtime sink。 |

最小实现方向：

1. 增加一个小 helper 判断当前 `ActiveKind::Transition` 是否 background-owned：`transition.target in {Idle, Standby}`，standby playback 同样算 background。
2. 增加一个 queue path helper：当 queue 到达 background transition/playback 时，复用现有 `startTransitionFromCurrentReferenceToUser()` 类路径，但不要把它用于 active user running。
3. 失败语义最小化：target user load/align/transition build 失败时 publish failed target，`stop_reason=None`；old background target 不写 user history；abort 当前 background transition 或进入 helper 已有的 safe idle/standby fallback；不恢复被抢占的 background transition。
4. 保持 interrupt path 的 current-frame rebuild 行为，并补 target=idle/standby/playback 测试。
5. 不改 `RuntimeBridge` priority，不改 stop watermark。
6. 不改 ET1 app，不改 public API，不把 synthetic transition 写成用户 run。
