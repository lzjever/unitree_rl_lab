...
bool hasEt1ConfigTreePath(const std::filesystem::path& path) {
  const std::filesystem::path normalized = path.lexically_normal();
  std::vector<std::string> parts;
  for (const auto& part : normalized) {
    parts.push_back(part.generic_string());
  ...
  constexpr const char* kEt1ConfigTree[] = {"deploy", "robots", "et1", "config"};
  constexpr std::size_t kNeedleSize =
      sizeof(kEt1ConfigTree) / sizeof(kEt1ConfigTree[0]);
  if (parts.size() < kNeedleSize) {
    return false;
  }
...
bool referencesEt1RuntimeDependency(const std::filesystem::path& path) {
  ...
  return hasEt1PolicyPath(path) || hasEt1ConfigPath(path) ||
         hasEt1ConfigTreePath(path) || hasEt1PolicyPath(canonical) ||
         hasEt1ConfigPath(canonical) || hasEt1ConfigTreePath(canonical);
}
