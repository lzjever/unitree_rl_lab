...
scripts/et1-action passive --password "<operator-provided-password>"  # explicit authorized passive mode only
scripts/et1-action urgent-stop --urgent
scripts/et1-action idle-load
scripts/et1-action idle-clear
...
Ordinary stop, pause, resume, relax, "停下", "停止动作", "恢复", and "放松" requests map to `standby` or `sequence-cancel`; these preserve idle config. Explicit completely-still/no-idle requests, including "不要动", "站着别动", "do not move", "completely still", "no idle", or "别播放 idle": `idle-clear`; do not use `urgent-stop` unless the user clearly asks for emergency stop, abort, kill, urgent/quick stop, "紧急停止", or "赶快停止". Use `fixstand` only when the user explicitly asks to enter the stand configuration. Use `passive --password ...` only when the user explicitly authorizes passive mode and provides the password; there is no default password lookup. Passive mode does not automatically recover, and the next normal recovery step is `fixstand`.
`standby` maps to tracker `POST /standby`, cancels active local sequences and tracker queued motions, but does not clear idle configuration. If idle motions are loaded, the tracker may enter idle background motion; if no idle is loaded, it stays in velocity0 standby. Trust the command JSON `state/ctrl/active/idle` fields instead of assuming pure velocity0; release status uses `ctrl:"standby"` for ordinary standby.
`idle-clear` maps to tracker `POST /idle {"paths":[]}` then `POST /standby`, cancels active local sequences and tracker queued motions, and should be used only when the user explicitly asks for completely still/no idle behavior. It does not use `/urgent_stop`.
`fixstand` and `passive` keep the compatibility `state:"fixstand"` or `state:"passive"`, but successful output also has `accepted:true` and `confirmed:false`; do not treat `state` alone as confirmation that `/status` has already reached the target state. `urgent-stop --urgent` maps to tracker `POST /urgent_stop`, returns `state:"urgent_stop"`, adds `urgent:true`, and is only for explicit urgent stop intent.
...
Default queue-ahead is `3`. This is a performance optimization, not a tracker playlist: the local sequence JSON remains the only editable sequence state, and already submitted tracker runs cannot be deleted, reordered, or replaced by `sequence-replace-tail`.
`sequence-replace-tail` only replaces unsubmitted local tail segments. If the requested tail has already been submitted, report the compact error with `error.code:"TAIL_ALREADY_SUBMITTED"`, `replaceable_count`, `submitted_count`, and the single `next:"sequence-interrupt"`.
When a later user message asks for a new action, call `run-text` or a new `sequence-start`; do not append to the old sequence unless the user explicitly says to append/continue after current work. For a known active sequence, use `sequence-interrupt --trk PATH` when the replacement `.trk` is ready; use `sequence-interrupt --text TEXT` or a new `run-text` for a generated replacement. The old local sequence must be canceled before new foreground intent proceeds.
Direct `run-text` / `run-trk` non-`--wait` success means accepted/submitted; it keeps the tracker returned `state` such as `queued`, sets `accepted:true` and `confirmed:false`, and does not mean the motion is running or complete. `--wait` can set `confirmed:true` when the final tracker state is `done` or `holding`; `failed`, `canceled`, or `stopped` return `TRACKER_RUN_FAILED`. Do not add holding detection, automatic standby, or retry workarounds around direct run submission.
`accepted` or submitted run ids mean the motion was accepted/submitted; accepted/submitted does not mean the robot has completed the motion. Use `sequence-status`, `GET /status?id=<run_id>`, or full `/status` before telling the user an action is actually done or holding.
