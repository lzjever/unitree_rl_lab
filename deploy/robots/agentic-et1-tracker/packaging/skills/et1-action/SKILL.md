...
Normal stop, do-not-move, stand-still, resume, relax, "停下", "不要动", "站着别动", "恢复", and "放松" requests map to `standby` or `sequence-cancel`; do not use `urgent-stop` unless the user clearly asks for emergency stop, abort, kill, urgent/quick stop, "紧急停止", or "赶快停止". Use `fixstand` only when the user explicitly asks to enter the stand configuration. Use `passive --password ...` only when the user explicitly authorizes passive mode and provides the password; there is no default password lookup. Passive mode does not automatically recover, and the next normal recovery step is `fixstand`.
