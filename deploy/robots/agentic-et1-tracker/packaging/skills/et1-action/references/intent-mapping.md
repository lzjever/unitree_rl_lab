...
- Replace later unsubmitted local work while current motion continues: `sequence-replace-tail`. If the requested tail has already been submitted to the tracker, do not claim success; return `TAIL_ALREADY_SUBMITTED` with `replaceable_count`, `submitted_count`, and `next:"sequence-interrupt"`.
...
- Immediately change a known active sequence to new text: `sequence-interrupt --text`; it cancels old tail work, uses standby while generating, then submits with tracker interrupt. A fresh `run-text` is also valid for new foreground intent and cancels any old local sequence first.
- Do not treat `accepted` or a submitted run id as action completion. Use `sequence-status`, `/status?id=<run_id>`, or full `/status` to confirm done/holding/running progress.
