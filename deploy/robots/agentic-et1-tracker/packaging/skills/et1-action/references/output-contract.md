...
For `standby`, `state` is `standby` for ordinary standby and may be `idle` when loaded idle motions immediately take over; `idle.enabled=true` means the idle configuration is still retained. Release tracker status uses `ctrl:"standby"` for ordinary standby. If the tracker does not reach a confirmable standby/idle state before the command timeout, output may fail with `error.code:"STANDBY_NOT_CONFIRMED"`.
...
For `idle-clear`, the CLI first sends tracker `POST /idle {"paths":[]}` and then requests ordinary `POST /standby`. Successful output uses `cmd:"idle-clear"`, `intent:"cancel_to_still"`, and the compact tracker `idle` fields after clearing idle config and confirming standby/idle. HTTP idle clear is any-state, but CLI `idle-clear` is not an any-state success guarantee: if the later standby request cannot be confirmed from passive, fault, fixstand, or another blocked state, the command may fail after the clear request was issued.
