...
Default success fields are short: `ok`, `cmd`, `intent`, `seq_id`, `state`, `ctrl`, `active`, `idle`, `segments`, `hold`, `motion_mode`, `executor`, and `next`.
...
`next` is always one command: `run-text`, `run-trk`, `sequence-start`, `sequence-status`, `sequence-append`, `sequence-replace-tail`, `sequence-cancel`, `sequence-interrupt`, `standby`, `fixstand`, `motion-mode`, `status`, `urgent-stop`, `idle-load`, or `cache-clear`.
