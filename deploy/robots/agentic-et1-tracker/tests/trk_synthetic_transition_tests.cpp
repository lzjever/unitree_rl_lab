...
TrkTrack makeSingleFrameTrack() {
  ...
  track.right_foot_contact_state.values.at(0) = 0;
...
TrkTrack makeTargetTrack() {
  ...
  track.left_foot_contact_state.values.at(0) = 2;
  track.right_foot_contact_state.values.at(0) = 1;
...
TEST_CASE("Trk synthetic transition builds required arrays and metadata in memory") {
  ...
  REQUIRE(track.metadata.duration_s == Catch::Approx(0.12));
  ...
  requireFloatArrayShape(track.joint_pos, {4, TrkSchema::kJointDim}, kJointDim);
  requireFloatArrayShape(track.joint_vel, {4, TrkSchema::kJointDim}, kJointDim);
  requireFloatArrayShape(track.body_pos_w, {4, TrkSchema::kBodyCount, 3}, kBodyCount * 3);
  requireFloatArrayShape(track.body_quat_w, {4, TrkSchema::kBodyCount, 4}, kBodyCount * 4);
  requireFloatArrayShape(track.body_lin_vel_w, {4, TrkSchema::kBodyCount, 3}, kBodyCount * 3);
  requireFloatArrayShape(track.body_ang_vel_w, {4, TrkSchema::kBodyCount, 3}, kBodyCount * 3);
  requireContactArrayShape(track.left_foot_contact_state, {4}, 1);
  requireContactArrayShape(track.right_foot_contact_state, {4}, 1);
  requireFloatArrayShape(track.ref_com_rel_navi, {4, 3}, 3);
  requireFloatArrayShape(track.ref_com_vel_navi, {4, 3}, 3);
  requireAllFloatsFinite(track);
}

TEST_CASE("Trk synthetic transition applies fixed interpolation policy") {
  const TrkTrack source_track = makeSingleFrameTrack();
  const TrkTrack target_track = makeTargetTrack();
  ...
  REQUIRE(atFrame(track.joint_pos, 1, 0) == Catch::Approx(12.0F));
  REQUIRE(atFrame(track.joint_pos, 2, 0) == Catch::Approx(14.0F));
  REQUIRE(atFrame(track.joint_pos, 3, 0) == Catch::Approx(16.0F));
  for (std::size_t frame = 0; frame < track.metadata.frames; ++frame) {
    REQUIRE(atFrame(track.joint_vel, frame, 0) == Catch::Approx(50.0F));
  }

  REQUIRE(atFrame(track.body_pos_w, 1, 0) == Catch::Approx(104.0F));
  for (std::size_t frame = 0; frame < track.metadata.frames; ++frame) {
    REQUIRE(atFrame(track.body_lin_vel_w, frame, 0) == Catch::Approx(100.0F));
  ...
  REQUIRE(atFrame(track.ref_com_rel_navi, 1, 0) == Catch::Approx(2.0F));
  REQUIRE(atFrame(track.ref_com_rel_navi, 2, 0) == Catch::Approx(3.0F));
  REQUIRE(atFrame(track.ref_com_rel_navi, 3, 0) == Catch::Approx(4.0F));
  for (std::size_t frame = 0; frame < track.metadata.frames; ++frame) {
    REQUIRE(atFrame(track.ref_com_vel_navi, frame, 0) == Catch::Approx(25.0F));
  }

  REQUIRE(contactAt(track.left_foot_contact_state, 0) == 1);
  REQUIRE(contactAt(track.left_foot_contact_state, 1) == 1);
  REQUIRE(contactAt(track.left_foot_contact_state, 2) == 2);
  REQUIRE(contactAt(track.left_foot_contact_state, 3) == 2);
  REQUIRE(contactAt(track.right_foot_contact_state, 0) == 0);
  REQUIRE(contactAt(track.right_foot_contact_state, 1) == 0);
  REQUIRE(contactAt(track.right_foot_contact_state, 2) == 1);
  REQUIRE(contactAt(track.right_foot_contact_state, 3) == 1);

  REQUIRE(atFrame(track.body_quat_w, 0, 0) == Catch::Approx(1.0F));
  REQUIRE(atFrame(track.body_quat_w, 1, 0) == Catch::Approx(0.894427F).margin(1.0e-5F));
  REQUIRE(atFrame(track.body_quat_w, 1, 1) == Catch::Approx(0.447214F).margin(1.0e-5F));
  REQUIRE(atFrame(track.body_quat_w, 2, 0) == Catch::Approx(0.447214F).margin(1.0e-5F));
  REQUIRE(atFrame(track.body_quat_w, 2, 1) == Catch::Approx(0.894427F).margin(1.0e-5F));
  REQUIRE(atFrame(track.body_quat_w, 3, 1) == Catch::Approx(1.0F));
}
...
TEST_CASE("Trk synthetic transition uses shortest-path quaternion nlerp") {
  ...
  REQUIRE(atFrame(transition->body_quat_w, 1, 0) ==
          Catch::Approx(0.955779F).margin(1.0e-5F));
  REQUIRE(atFrame(transition->body_quat_w, 1, 1) ==
          Catch::Approx(0.294086F).margin(1.0e-5F));
  REQUIRE(atFrame(transition->body_quat_w, 3, 0) == Catch::Approx(0.6F).margin(1.0e-5F));
  REQUIRE(atFrame(transition->body_quat_w, 3, 1) == Catch::Approx(0.8F).margin(1.0e-5F));
}
