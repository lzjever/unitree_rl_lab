...
LocoLowerDeployConfig validLocoLowerConfig() {
...
void writeWrongLocoLowerOutputShapeOnnx(const std::filesystem::path& path) {
  static constexpr unsigned char kModel[] = {
      };
  std::ofstream out(path, std::ios::binary);
  REQUIRE(out);
  out.write(reinterpret_cast<const char*>(kModel), sizeof(kModel));
...
void requireLocoLowerRuntimeRejects(
    const OnnxLocoLowerPolicyRuntimeConfig& config,
    const std::string& message) {
  try {
    OnnxLocoLowerPolicyRuntime runtime(config);
  } catch (const PolicyRuntimeError& err) {
    REQUIRE(std::string(err.what()).find(message) != std::string::npos);
    return;
  }
...
TEST_CASE("OnnxLocoLowerPolicyRuntime constructs and runs a minimal loco lower ONNX policy") {
  TempDir tmp;
  const auto model = tmp.root / "minimal-loco-lower.onnx";
  writeMinimalLocoLowerOnnx(model);

  OnnxLocoLowerPolicyRuntime runtime({model, validLocoLowerConfig()});

  const Vec actions = runtime.infer({Vec(kLocoLowerPolicyObsDim, 0.0F)});

  REQUIRE(actions.size() == kLocoLowerPolicyJointDim);
  REQUIRE(std::all_of(actions.begin(), actions.end(),
                      [](float value) { return std::isfinite(value); }));
  REQUIRE(std::all_of(actions.begin(), actions.end(),
                      [](float value) { return value == 0.0F; }));
...
TEST_CASE("OnnxLocoLowerPolicyRuntime rejects wrong input shape metadata") {
  TempDir tmp;
  const auto model = tmp.root / "wrong-loco-lower-input.onnx";
  writeWrongLocoLowerInputShapeOnnx(model);

  requireLocoLowerRuntimeRejects({model, validLocoLowerConfig()},
                                 "obs shape must be [1,45]");
}

TEST_CASE("OnnxLocoLowerPolicyRuntime rejects wrong output action dimension metadata") {
  TempDir tmp;
  const auto model = tmp.root / "wrong-loco-lower-output.onnx";
  writeWrongLocoLowerOutputShapeOnnx(model);

  requireLocoLowerRuntimeRejects({model, validLocoLowerConfig()},
                                 "actions shape must be [1,12]");
}

TEST_CASE("OnnxVelocityPolicyRuntime still rejects a loco lower model contract") {
  TempDir tmp;
  const auto model = tmp.root / "minimal-loco-lower.onnx";
  writeMinimalLocoLowerOnnx(model);

  requireVelocityRuntimeRejects({model, validVelocityConfig()},
                                "obs shape must be [1,225]");
}
